    ...
    <div className="dashboard-grid">
      <div className="dash-main">
        <section className="usual-commute-card">
          ...
          <div className="usual-route">
            ...
            <div className="usual-time"><span><CalendarDays/>Mon—Fri</span><span><Clock3/>7:00 AM</span><span><User/>1 seat</span></div>
          </div>
      ...
      <aside className="dash-aside">
        <section className="next-trip-card">
          ...
          <div className="next-route"><div><i/><span><small>PICKUP · 6:55 AM</small><strong>Novare Mall, Sangotedo</strong></span></div><em/><div><i/><span><small>DROP-OFF · 7:42 AM</small><strong>Ozumba Mbadiwe, VI</strong></span></div></div>
...
function CompactRide({ ride, onClick }: { ride: Ride; onClick: () => void }) {
  return <button className="compact-ride" onClick={onClick}>
    ...
    <div className="compact-route"><strong>{ride.time}</strong><span>{ride.from} <ArrowRight size={12}/> {ride.to}</span><small><Repeat2 size={12}/> {ride.recurring ? 'Every weekday' : 'One-off ride'}</small></div>
...
function ExploreRides({ onRide, onSearch }: { onRide: (r: Ride) => void; onSearch: () => void }) {
  ...
  const allRides = useMemo(() => [...rides, {...rides[0], id:'PG-4388', driver:'Damilola James', initials:'DJ', match:83, price:1100, time:'7:25 AM', community:'Chevron Estate', avatarColor:'#bd8352'}], []);
  return <>
  ...
  return <>
    ...
    <section className="active-trip-panel">
      ...
      <div className="trip-detail-side">
        <div className="trip-status"><span><i/>DRIVER EN ROUTE</span><small>Trip #PG-2841</small></div>
        ...
        <div className="trip-route-detail"><div><span className="trip-time">6:55</span><i/><span><small>PICKUP</small><strong>Novare Mall main entrance</strong><em>Sangotedo, Lagos</em></span></div><b/><div><span className="trip-time">7:42</span><i/><span><small>DROP-OFF</small><strong>Sterling Towers</strong><em>Marina, Victoria Island</em></span></div></div>
