/**
 * COMUTA mock data seed — ported from PWA src/data/seed.ts.
 * Deterministic local data that makes the app behave like a live backend.
 * Dates are generated relative to "today" so the demo stays fresh.
 */
import type {
  AppNotification,
  AppState,
...
import { estimateDurationMin, estimateRoadDistance } from '../utils/geo';
...
function makeTrip(
  ...
  const distanceKm = estimateRoadDistance(from.lat, from.lng, to.lat, to.lng);
